...
export default class Logs extends BaseCommand {
  static args = {
    service: Args.string({
      description: 'Service to show logs for (ipfs, cluster, traefik, nginx)',
      options: ['ipfs', 'cluster', 'traefik', 'nginx'],
      required: false,
    }),
  }
  ...
  async run(): Promise<void> {
    ...
    try {
      const services = args.service ? [args.service] : ['ipfs', 'cluster', 'traefik', 'nginx']
